...
# use ?= syntax to allow WANTs to be provided on command line
...
# build configurations (make config=[debug,tool,player])
#   debug  - full debug of player and tool portions of the engine (-O0)
#            mainly for use in-house by Lutro engine developers.
#   tool   - provides logging, assertion checking, special LUA APIs for content creators.
#            this is the default config and is how releases of Lutro should be shipped to Love2D devs.
#   player - most assertions disabled or demoted to logs, special tooling APIs disabled.
...
# standard debugging switch, mostly for general Makefile API compatiblity.
# When DEBUG=1, LUTRO_CONFIG is forced to "debug".
ifeq ($(DEBUG), 1)
	LUTRO_CONFIG = debug
endif
...
$(lutro_sources_file): Makefile Makefile.common
